      ...
      <v-toolbar flat color="white">
        ...
        <v-dialog v-model="dialog" persistent max-width="600px">
          <v-card>
            <v-card-title>
              <span>{{
                edit === true ? "تعديل كود اللون" : "إضافة كود اللون جديد"
              }}</span>
              <v-spacer></v-spacer>
            ...
            >
              ...
              >
                ...
                <v-card-text>
                  <div>
                    <span>الكود: </span>{{ admin.code }}
                    <span :style="{'background': admin.code}" style="width: 16px; height: 16px; margin-inline: 5px; display: inline-block; border-radius: 4px;"></span>
                  </div>
                </v-card-text>
      ...
      >
        ...
        <template v-slot:[`item.code`]="{ item }">
          <div v-if="item.code != null">
            <span>{{ item.code }}</span>
            <span :style="{'background': item.code}" style="width: 16px; height: 16px; margin-inline: 5px; display: inline-block; border-radius: 4px;"></span>
          </div>
          <v-chip v-else small color="secondary" dark>غير متوفر</v-chip>
        </template>
...
export default {
  ...
  computed: {
    formTitle() {
      return this.edit ? "Edit city" : "Add city";
    },
  },
  watch: {
    dialog(val) {
  ...
  methods: {
    ...
    fetch() {
      this.getDataFromApi().then((data) => {
        ...
        let meta = data.meta;
        console.log('getDataFromApi meta >>', data.meta);
        ...
        this.$http
          ...
          .then((res) => {
            if (res.data.status.status) {
              ...
              this.alert.message = "Edit city done";
              this.close();
              this.errors = [];
              this.admin = {
                id: null,
        ...
        this.$http
          ...
          .then((res) => {
            if (res.data.status.status) {
              ...
              this.alert.message = "Add city done!";
              this.close();
              this.errors = [];
              this.admin = {
                id: null,
