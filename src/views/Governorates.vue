      ...
      <v-toolbar flat color="white">
        ...
        <v-dialog v-model="dialog" persistent max-width="600px">
          <v-card>
            <v-card-title>
              <span>{{
                edit === true ? "تعديل محافظة" : "إضافة محافظة جديد"
              }}</span>
              <v-spacer></v-spacer>
...
export default {
  ...
  computed: {
    formTitle() {
      return this.edit ? "Edit governorate" : "Add governorate";
    },
  },
  watch: {
    dialog(val) {
        ...
        this.$http
          ...
          .then((res) => {
            if (res.data.status.status) {
              ...
              this.alert.message = "Edit governorate done";
              this.close();
              this.errors = [];
              this.admin = {
                id: null,
        ...
        this.$http
          ...
          .then((res) => {
            if (res.data.status.status) {
              ...
              this.alert.message = "Add governorate done!";
              this.close();
              this.errors = [];
              this.admin = {
                id: null,
